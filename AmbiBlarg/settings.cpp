...
Settings::Settings()
{
    dataFolder = QCoreApplication::applicationDirPath() + "/ambiblarg_data/";
...
{
    QFile file(getDataFolderPath() + "languages/" + languageName + "/translations.txt");

    if(!file.open(QIODevice::ReadOnly))
    {
        QMessageBox::information(NULL, "AmbiBlarg", "The language \""+languageName+"\" is not present in the /ambiblarg_data/languages/ folder.\n\nEnglish will be selected as fallback language.");
    ...
    {
        QString path = getDataFolderPath() + "languages/" + languageName + "/"+ transFile;
        if(!QFile(path).exists())
            path = getDataFolderPath() + transFile;
...
void Settings::setupLanguageSelector(QListWidget* selector)
{
    ...
    QDir translationsFolder(getDataFolderPath() + "languages/");
...
void Settings::setLanguage(QString language)
{
    ...
    QMessageBox::information(NULL, "AmbiBlarg", QString("The Language has been changed to %1.\n\nPlease restart AmbiBlarg for the changes to take effect.").arg(language));
}
